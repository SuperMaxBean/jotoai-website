...
const ProductIntro: React.FC = () => {
  ...
  const featureList = [
    t("提示词越狱检测", "Prompt Jailbreak Detection"),
    t("PII 识别与遮掩", "PII Detection & Masking"),
    t("违规内容过滤", "Content Compliance Filtering"),
    t("恶意链接检测", "Malicious URL Detection"),
    t("黑白名单管控", "Blacklist/Whitelist Control"),
    t("安全日志与审计", "Security Logs & Audit"),
  ];
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-16 max-w-4xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold mb-6 text-white">
            {t('唯客 AI 护栏是什么？', 'What is JOTO AI Guardrails?')}
          </h2>
          <p className="text-lg text-gray-300 leading-relaxed">
            {t(
              '唯客 AI 护栏是一款专为中国企业设计的大模型应用运行时安全防护系统。它以',
              'JOTO AI Guardrails is a runtime security system designed specifically for enterprise LLM applications. Powered by a '
            )}
            <span className="text-brand-green">{t('极低延迟的流式检测引擎', 'ultra-low latency streaming detection engine')}</span>
            {t(
              '为核心，在用户输入和模型输出的双向数据流中，实时执行安全策略，同时提供完整的安全日志看板。',
              ', it enforces security policies in real-time across bidirectional data flows of user input and model output, while providing a complete security dashboard.'
            )}
          </p>
        </div>
        ...
        >
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-0">
                ...
                <div className="flex flex-col items-center gap-4 group shrink-0 w-full lg:w-auto">
                     ...
                     <span className="text-slate-400 font-medium text-sm">{t('用户请求', 'User Request')}</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2 shrink-0 relative group z-10 w-full lg:w-auto px-4 lg:px-0">
                    <div className="w-full lg:w-60 xl:w-64 h-28 lg:h-32 bg-gradient-to-br from-brand-blue to-blue-700 rounded-xl shadow-[0_10px_40px_-10px_rgba(46,124,246,0.5)] flex flex-col items-center justify-center gap-2 lg:gap-3 relative overflow-hidden transform group-hover:scale-105 transition-transform duration-300 border border-white/10">
                        ...
                        <span className="text-white font-bold text-base lg:text-lg tracking-wide">{t('输入安全检测', 'Input Guard')}</span>
                    </div>
                ...
                <div className="flex flex-col items-center gap-4 shrink-0 w-full lg:w-auto">
                    ...
                    <span className="text-slate-400 font-medium text-sm">{t('核心引擎', 'Core Engine')}</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2 shrink-0 relative group z-10 w-full lg:w-auto px-4 lg:px-0">
                     <div className="w-full lg:w-60 xl:w-64 h-28 lg:h-32 bg-gradient-to-br from-brand-blue to-blue-700 rounded-xl shadow-[0_10px_40px_-10px_rgba(46,124,246,0.5)] flex flex-col items-center justify-center gap-2 lg:gap-3 relative overflow-hidden transform group-hover:scale-105 transition-transform duration-300 border border-white/10">
                        ...
                        <span className="text-white font-bold text-base lg:text-lg tracking-wide">{t('输出安全检测', 'Output Guard')}</span>
                    </div>
                    <span className="text-[10px] text-brand-blue font-bold tracking-[0.2em] uppercase mt-2 lg:mt-3">Guard Layer</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-4 group shrink-0 w-full lg:w-auto">
                    ...
                    <span className="text-slate-400 font-medium text-sm">{t('安全响应', 'Safe Response')}</span>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 px-4">
             ...
             <div className="text-center mt-12 flex items-center justify-center gap-3 text-brand-blue font-bold tracking-wider text-sm md:text-base opacity-80">
                ...
                {t('流式检测 · 双向防护 · 毫秒响应', 'Streaming Detection · Bidirectional Protection · Millisecond Response')}
             </div>
        </div>
      </div>
    </section>
