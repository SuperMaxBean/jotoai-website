...
// 配置 marked：GitHub 风格，内嵌 HTML 直接透传（混合 markdown + HTML 场景安全）
marked.setOptions({ gfm: true, breaks: false });

// LLM 常见的前言/包装词，需要从正文剥离
const LLM_PREAMBLE_RE = /^\s*(?:改写后的文本|改写后的文章|改写结果|以下是改写后的(?:文本|文章|内容)|Here(?:'|')?s?\s+the\s+rewritten(?:\s+(?:text|article|version))?|Rewritten\s+(?:text|article|version))\s*[:：]?\s*\n*/i;
...
/**
 * 将内容统一转换为 HTML（无论输入是 markdown、HTML、还是混合）。
 * 使用 marked 库：HTML 块原样透传，markdown 块被正确解析。
 * 幂等：对纯 HTML 输入产出等价 HTML；对 markdown 产出正确 HTML；
 * 对混合内容正确处理每一段。
 */
function markdownToHtml(text) {
  if (!text) return '';
  // 先剥离常见 LLM 前言/包装，避免"改写后的文本:"之类标签泄漏进正文
  const stripped = text.replace(LLM_PREAMBLE_RE, '');
...
/**
 * 清洗 LLM 输出：只去除文档级标签和代码块包裹。
 * 不再手动包裹 <p>、不再拼接 —— 这些交给 marked，避免把 markdown 行误包进
 * HTML 块导致后续无法解析（marked 对 HTML 块内部内容原样透传）。
 */
function cleanHtml(html) {
  if (!html) return '';
  let cleaned = html.trim();
  // 去除 ```html / ```markdown ... ``` 包裹
  cleaned = cleaned.replace(/^```(?:html|markdown)?\s*/i, '').replace(/\s*```$/, '').trim();
  // 去除 LLM 前言/包装词（"改写后的文本:"等）
  cleaned = cleaned.replace(LLM_PREAMBLE_RE, '');
  // 去除完整 HTML 文档结构标签
...
async function rewriteArticle(originalArticle, keyword, llmConfig, rewriteRounds = 3, wordCount = 1000, customPrompt = null) {
  try {
    ...
    // 统一通过 marked 转换：HTML 透传、markdown 解析、混合内容正确处理。
    // 不再用 includes('<p>') 等脆弱判断 —— LLM 常返回混合内容，子串判断会漏转。
